...
void randomSampleCuts(){

  // 1. Load each Tree individually 
  std::string inFolder = "fullSampleCuts/";
  std::string outFolder = "randomSampleCuts/";
...
void selectRandom(std::string inFolder, std::string outFolder, std::string filename){
  ...
  // Check which one has less entries, to sampling over the others

  int sample_size = (int)back1Entries;
  // back1 fixed
  ...
  std::uniform_int_distribution<int> distribution1(0,signalEntries);
  ...
  for (int sample_i = 0; sample_i < sample_size; ++sample_i){
    treeBack1->GetEntry(sample_i);
    totalSimulation->Fill();
    if(!(sample_i%100)){
      cout << "Entry_i : "<< sample_i << endl;
    } 
